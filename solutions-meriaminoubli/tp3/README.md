...
gcloud run deploy <my-app-name> \
    --image=europe-west1-docker.pkg.dev/dauphine-437611/dauphine-ar/meriamin-api:latest \
    --platform=managed \
    --region=europe-west1 \
...
# Replace docker buildx build --platform linux/amd64 with docker build -t if it does not work
docker buildx build --platform linux/amd64 --push -t europe-west1-docker.pkg.dev/dauphine-437611/dauphine-ar/mi-app3:latest -f Dockerfile .

gcloud run deploy mi3-streamlit \
    --image=europe-west1-docker.pkg.dev/dauphine-437611/dauphine-ar/mi-app3-streamlit:latest \
    --platform=managed \
    --region=europe-west1 \
    --allow-unauthenticated \
    --port 8080
```
gcloud run deploy mi-streamlit3 --image=europe-west1-docker.pkg.dev/dauphine-437611/dauphine-ar/mi-app3-streamlit:latest --platform=managed --region=europe-west1 --allow-unauthenticated --port 8080
