...
def merge_documents_by_page(documents: list[Document]) -> list[Document]:
    ...
    for (document_source, page_number), docs in pages.items():
        if docs:
...
def create_cloud_sql_database_connection() -> PostgresEngine:
    ...
    engine = PostgresEngine.from_instance(project_id=PROJECT_ID,instance=INSTANCE,region=REGION,database=DATABASE,user=DB_USER,password=DB_PASSWORD,)
    return engine
...
def get_vector_store(engine: PostgresEngine, table_name: str, embedding: VertexAIEmbeddings) -> PostgresVectorStore:
    ...
    """
    vector_store = PostgresVectorStore.create_sync(engine=engine,table_name=table_name,embedding_service=embedding,)
    return vector_store
...
async def main():
    ...
    try:
        ...
        local_filepath = download_file_from_bucket(bucket, file_path, DOWNLOADED_LOCAL_DIRECTORY)
        ...
        documents = read_file_from_local(os.path.join(DOWNLOADED_LOCAL_DIRECTORY, os.path.basename(local_filepath)))
