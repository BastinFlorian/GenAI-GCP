...
# liste les fichiers dans un répertoire spécifié d'un bucket Google Cloud Storage
...
# télécharge un fichier depuis un bucket Google Cloud Storage vers un répertoire local
...
# fusionne des objets Document basés sur le numéro de page dans leurs métadonnées
...
async def create_table_if_not_exists(
        table_name: str, engine: PostgresEngine
        ) -> None:
    """
...
async def main():
    ...
    try:
        ...
        assert os.path.exists(
            os.path.join(DOWNLOADED_LOCAL_DIRECTORY, os.path.basename(
                file_path))
        ), "File not downloaded successfully"

        # Test read_file_from_local
        documents = read_file_from_local(os.path.join(
            DOWNLOADED_LOCAL_DIRECTORY, os.path.basename(local_filepath)))
